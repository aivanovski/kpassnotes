...
class FilePickerInteractor(
    ...
    suspend fun copyToPrivateStorage(file: FileDescriptor): OperationResult<FileDescriptor> =
        withContext(dispatchers.IO) {
            val readFileResult = openFile(file)
            if (readFileResult.isFailed) {
                return@withContext readFileResult.mapError()
            }
            ...
            val openWriteResult = fsProvider.openFileForWrite(
                dstFile,
                OnConflictStrategy.CANCEL,
                FSOptions.DEFAULT
            )
            if (openWriteResult.isFailed) {
                return@withContext openWriteResult.mapError()
            }

            val copyResult = InputOutputUtils.copy(
                from = readFileResult.getOrThrow(),
                to = openWriteResult.getOrThrow(),
                isClose = true
            )
